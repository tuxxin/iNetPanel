...
restore_firewall() {
    ...
    # Remove WG UDP port
    firewall-cmd --permanent --remove-port=${WG_PORT:-1443}/udp 2>/dev/null
...
# Restore SSH to listen on all interfaces
SSHD_CONF="/etc/ssh/sshd_config"
if [ -f "$SSHD_CONF" ]; then
    sed -i '/^ListenAddress/d' "$SSHD_CONF"
    systemctl reload sshd 2>/dev/null
    echo -e "  ${GREEN}SSH: Restored to listen on all interfaces${NC}"
fi
